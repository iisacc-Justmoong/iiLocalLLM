...
int main(int argc,char** argv){QCoreApplication app(argc,argv);
    if(argc<3||argc>6)return 2;bool toolGrammar=true,qwenSampling=false,thinking=false;
    for(int i=3;i<argc;++i){const auto option=QString::fromLocal8Bit(argv[i]);
        ...
        else if(option=="--thinking"){thinking=true;qwenSampling=true;}else return 2;}
    try {
        ...
        const auto path=work+"/book.ipynb";write(path,QJsonDocument(notebook).toJson());
        ...
        auto policy=std::make_shared<a::RulePolicy>(a::PermissionMode::DontAsk,QList<a::PermissionRule>{{"NotebookEdit",a::PermissionBehavior::Allow}});
        ...
        const QJsonObject report{{"passed",passed},{"checks",checks},{"model",uri},{"model_requests_unmodified",true},{"marker",marker},{"answer",a::toJson(result)},
