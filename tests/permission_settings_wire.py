...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="settings-wire-") as temporary:
        ...
        def policy(allow=True):
            policy_file.write_text(json.dumps({"allowManagedPermissionRulesOnly": True, "permissions": {
        ...
        for value in ([], {"enabled_sources": ["managed"]}, {"mode": "auto"}, {"settings": {"permissions": {"deny": [1]}}},
                      {"settings": {"permissions": {"additionalDirectories": ["outside"]}}}, {"unrecognized": True}):
            ...
            if args.catalog:
                ...
                for allowed in (True, False):
                    ...
                    if allowed:
                        ...
                        assert not (workspace / path).exists() and any(m.get("is_error") and "permission denied" in m["text"].lower() for m in tool_results), outcome
