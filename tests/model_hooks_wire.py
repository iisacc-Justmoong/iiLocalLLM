"""Prompt hook host validation; optional real-model API/CLI/MCP qualification."""
import argparse
import asyncio
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="mh-") as temporary:
        ...
        def group(condition, matcher=None):
            result = {"hooks": [{"type": "prompt", "prompt": condition + "\nHook input: $ARGUMENTS", "timeout": 60}]}
            if matcher:
        ...
        assert unavailable.returncode and b"Prompt hooks require --model and --models" in unavailable.stderr, unavailable
        ...
        if args.catalog:
            ...
            with server("api", common_daemon + ["--agent-hooks", hooks, "--agent-permission-settings", policy, "--config", model_config, "--agent-no-auto-compact"], r"iiLocalLLM HTTP: http://127\.0\.0\.1:(\d+)") as match:
                ...
                for name, prompt, expected in (
                    ...
                    ("blocked", 'Call Write exactly once with path="blocked.txt" and content="NO". Then reply DONE.', "cancelled"),
                    ("stop", 'Reply with STOP_MARKER. Do not use tools.', "cancelled"),
                    ...
                    diagnostics = [f["data"]["data"] for f in frames if f.get("event") == "rpc" and f["data"]["event"] == "hook" and f["data"]["data"].get("hook_type") == "prompt"]
            ...
            with server("mcp", common_mcp + mcp_flags + ["--http-port", "0", "--credentials", credentials, "--state", str(root / "mcp-state")], r'\{"endpoint":"([^"\n]+)"\}') as match:
                ...
                for path in ("mcp-allowed.txt", "mcp-blocked.txt", "host-denied.txt"):
                    ...
                    diagnostics = [f["params"]["_meta"]["iisacc/agentEvent"]["data"] for f in frames if f.get("method") == "notifications/progress" and f["params"].get("_meta", {}).get("iisacc/agentEvent", {}).get("data", {}).get("hook_type") == "prompt"]
