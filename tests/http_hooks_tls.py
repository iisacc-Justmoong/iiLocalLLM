"""Independent TLS peer checks pinned-IP Host/SNI and certificate verification."""
...
def main():
    ...
    for key in ("DYLD_LIBRARY_PATH", "DYLD_FRAMEWORK_PATH", "DYLD_FALLBACK_LIBRARY_PATH", "LIBRARY_PATH"):
        env.pop(key, None)
    with tempfile.TemporaryDirectory(prefix="hh-") as temporary:
        root = Path(temporary)
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                body = json.dumps({"hookSpecificOutput": {"hookEventName": "PreToolUse", "additionalContext": json.dumps(record)}}).encode()
        ...
        try:
            ...
            def probe(host, ca):
                result = subprocess.run([executable, f"https://{host}:{port}/hook", str(ca)], cwd=root, env=env,
                                        capture_output=True, text=True, check=True, timeout=15)
                return json.loads(result.stdout)

            trusted = probe("localhost", cert)
            assert trusted["diagnostics"][-1]["outcome"] == "success", trusted
            record = json.loads(trusted["feedback"])
            assert record["sni"] == "localhost", record
            assert record["host"] == f"localhost:{port}", record
            assert record["body"]["tool_input"]["content"] == "TLS_INPUT", record
            for result in (probe("127.0.0.1", cert), probe("localhost", "-")):
                assert result["diagnostics"][-1]["outcome"] == "non_blocking_error" and not result["feedback"], result
