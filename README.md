...
0.26.0은 승인된 권한의 파일 저장과 세션별 갱신을 제공한다. 규칙·모드·추가 디렉터리를 적용하고 clear/fork/자식 접수에 런타임 권한을 이어 준다. 재시작·클라이언트 격리·저장 실패의 경계는 [PermissionUpdates.md](docs/PermissionUpdates.md)를 따른다.
...
C++20, Qt 6.8.3 Core/Network 기반 로컬 LLM 서비스 SDK이다. 버전은 0.26.0이다. 앱은 `model://id`로 모델을 사용한다. 서비스는 manifest와 설치 파일을 관리하고 시작 시 검사한 하드웨어에 따라 실행 장치를 자동 선택한다. 모델 실행은 llama.cpp 또는 MLX에 맡기고 세션, 프롬프트 예산, KV 캐시, FIFO 스케줄링, 스트리밍, 로컬 IPC를 관리한다. 기존 `helloWorld()`와 `iiLocalLLM::iiLocalLLM` CMake 타깃은 유지한다.
