...
PromptHookResult evaluatePromptHook(const PromptHook& hook,const HookInput& input,const QByteArray& payload,
    ...
    request.responseSchema={{"type","object"},{"properties",QJsonObject{{"ok",QJsonObject{{"type","boolean"}}},{"reason",QJsonObject{{"type","string"}}}}},
    ...
    require(error.error==QJsonParseError::NoError&&document.isObject(),"Prompt hook must return a JSON object");const auto object=document.object();
    for(auto it=object.begin();it!=object.end();++it)require(it.key()=="ok"||it.key()=="reason","Unknown prompt hook response field");
    require(object["ok"].isBool()&&(!object.contains("reason")||object["reason"].isString()),"Prompt hook response does not match the decision schema");
    PromptHookResult result;result.usage=reply.usage;result.model=request.model;
    if(!object["ok"].toBool()) {
        result.result.block=true;result.result.stop=true;result.result.stopReason=object["reason"].toString("Prompt hook condition was not met");
        result.result.feedback="Prompt hook condition was not met: "+result.result.stopReason;
    }
    return result;
}
}
