...
0.51의 팀원 SendMessage 스키마는 현재 미결 종료 요청에 맞춰 매 턴 갱신된다. 도구 목록을 생성한 뒤 상태가 바뀌더라도 실행 시 요청 소유권을 다시 검사한다. 일반 문자열은 그대로 전달하며 구조화된 종료 응답에는 실제 요청 ID와 필요한 필드를 요구한다. 직접 C++ 팀 호출도 공개 도구 목록과 같은 추가 도구 공급자를 사용한다. 상세 계약은 [Teams.md](Teams.md)에 있다.

0.52는 [로컬 플러그인](Plugins.md)의 버전 선택 저장소와 실행기 구성을 제공한다. 인증 API의 agent.plugins.list와 MCP의 iiLocalLLM.agent.plugins.list로 활성 snapshot을 확인한다. 하위 에이전트는 구성된 옵션을 상속하며 프로필 필터 이후 재등록하지 않는다.
