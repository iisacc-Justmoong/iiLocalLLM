...
RpcHandler.isControlMethod는 신뢰된 호스트가 분류한다. 기본은 false이다. agent::Api는 agent.permissions.pending/respond와 agent.cancel/status를 제어 메서드로 지정한다. 자격 증명과 인자 검증은 기존 dispatch에서 수행한다. JSON의 역할·메타데이터·이름 접두사로 제어 용량을 획득할 수 없다. 모델 생성과 /v1/models도 일반 용량에 포함한다.

MCP HTTP의 maxStreams와 maxStreamsPerSession은 일반 활성/보관 스트림을 제한한다. 보관 개수에는 background 스트림이 포함된다. maxControlStreams는 별도 전역 활성 제어 스트림 한도와 연결별 보관 제어 스트림 한도이며 기본 4, 범위 1–64이다. 내부 작업자는 maxStreams + maxControlStreams + 4까지 증가한다. 등록된 ServerOptions.controlHandlers 메서드와 ping만 제어 응답 스트림을 사용한다. 일반 요청이 섞인 2025-03-26 배치는 일반 용량을 사용한다. 알림·역방향 응답·DELETE는 기존처럼 응답 스트림을 만들지 않는다.
...
공개 C++ 옵션 구조체와 RpcHandler 가상 인터페이스가 변경되었다. C++ 소비자는 0.28 헤더와 라이브러리를 함께 사용해 다시 빌드해야 한다. 기존 바이너리에 라이브러리만 교체하는 호환성을 보장하지 않는다.
