...
0.46 검증에서 종료 훅의 예외 복구와 짧은 시간 예산 검사를 분리했다. 공유 시간 예산에는 실행 문맥 준비도 포함되므로 40ms 안에 특정 콜백이 반드시 시작한다고 가정하지 않는다. 예외 뒤 후속 훅 실행과 자식의 주 세션 종료 훅 제외는 여유 있는 예산에서 확인하고, 40ms 사례는 종료 완료·timeout·후속 콜백 차단을 검증한다. 제품의 SessionEnd 시간 예산은 변경하지 않는다.
