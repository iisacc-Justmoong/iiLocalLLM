...
500,000 단계, 100ms 협력식 매칭 기한, 재귀 깊이 128을 적용한다. 패턴 집합 준비와
파일/디렉터리 메타데이터 조회를 마친 뒤 매칭 기한을 시작하므로 파일 시스템 대기 시간을
패턴 계산 비용으로 오인하지 않는다. 메타데이터는 각 검사에서 다시 조회한다. 초과는 ResourceLimit이며
