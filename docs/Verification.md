...
| 전송 | 소스·설치본 각각 인증 HTTP·IPC·MCP HTTP·공식 MCP 1.26.0 stdio 통과 |
| 설치 동일성 | 공개 헤더 47개, 실행 진입점 4개의 버전 0.38.0, 라이브러리 해시·UUID, 문서·QML·카탈로그·실제 로더 경로 |

초기 공개 인터페이스의 링크 실패, 회상 잘림이 모델 본문에는 표시되지 않던 실패, 후보 경로 enum과 입력 예산 회귀 실패를 먼저 기록했다. 본문에도 잘림을 표시하고 후보 목록과 JSON Schema의 enum을 같은 입력 예산으로 제한했다. 모델이 스키마를 따르지 않아도 호스트의 경로 검증은 계속 적용한다. 첫 fixture 컴파일의 콜백 타입 오류는 제품 결함과 구분하며 해당 로그도 보존한다.

실제 Qwen의 첫 자동 회상은 성공했지만 명시적 회상은 실패했다. 응답 진단을 추가한 재현에서 `unknown_selected_path`를 확인했고, 후보 경로를 enum으로 제한한 뒤 명시적 회상이 통과했다. 이어 비활성 대조에서 `</think>`와 `MISSING`이 함께 출력됐다. 비활성 프롬프트가 이미 닫은 추론 경계를 모델이 반복한 경우에만 upstream 파서의 prefill을 보정했다. 답변 중간·코드·JSON에 있는 태그와 생각 출력이 켜진 요청은 보정하지 않는 회귀를 추가했다. fixture에서 응답을 치환하거나 성공 조건을 완화하지 않았다.

첫 전체 Release의 기존 HTTP 훅 검사에서는 CLI 시작 중 dyld `Allocator.cpp` assertion이 발생했다. 원본 실패를 보존했고 단독 재실행 3회와 후속 전체 검사에서 통과했다. 원인은 확정하지 않았으며 이를 제품 코드 수정으로 해결했다고 주장하지 않는다. 각 수정 이후 다시 고정한 최종 소스로 위의 전체 검사를 수행했다.

단위·엔진 회귀는 본문을 선택 모델에 보내지 않는 계약, 인덱스·관측 경로 제외, 잘못된 JSON과 경로, UTF-8 바이트·줄·전체 문맥 한도, 변경된 파일 생략, 부분 읽기의 Edit 거부, 중복 회상 방지와 압축 후 재회상, 긴급 입력의 이전 선택 취소, 본 대화 종료 뒤 첨부 방지를 검사한다. 인증 API의 client ID 분리와 MCP 연결 소유권, 회상 비활성 설정도 확인했다. 전송 fixture는 빈 후보 목록의 제어 경로를 검증하며 실제 추론 검사는 별도로 수행한다.

실제 모델 fixture는 비어 있는 MEMORY.md와 두 주제를 만들고, 선택 입력에는 없는 임의 검증 코드를 관련 주제 본문에만 넣는다. 선택 모델 호출은 수정하지 않은 ServiceModel 요청이며 선택된 메모에서 본 모델이 정확한 코드를 반환했다. 비동기 결과를 회수할 도구 경계를 만들기 위해 본 대화의 Read 선택과 답변 지침은 호스트가 유도한다. 자유로운 전체 작업 수행의 검증은 아니다. 명시적 회상은 새 빈 대화의 기록을 변경하지 않고 메모를 반환했고, 회상을 끈 대조는 `MISSING`과 선택 토큰 0을 반환했다.

소스 자동 실행은 2턴·본 모델 56 생성 토큰·선택 모델 17 생성 토큰, 설치본은 2턴·본 모델 60 생성 토큰·선택 모델 17 생성 토큰이었다. 명시적 선택은 각각 17 생성 토큰, 비활성 대조는 각각 1턴·5 생성 토큰이었다. 모델은 `model://qwen3-8b-q4`, GGUF 5,027,783,488바이트, SHA-256 `d98cdcbd03e17ce47681435b5150e34c1417f50b5c0019dd560e4882c5745785`이며 매 로딩에서 manifest 무결성을 검사한다.

최종 검증 입력 360개 파일의 SHA-256을 고정하고 검사 후 같은 소스임을 확인했다. 이후 이 검증 문서만 갱신하고 설치 문서를 다시 대조한다. 설치 prefix는 `build/memory-recall-stage`, 소비자 빌드는 `build/memory-recall-consumer/build`이다. Release 라이브러리 SHA-256 `2c3c29025a3bb27b15fc7e9ed676bcbb3a6180857b05c747f7346728ccb3d823`은 stage와 동일하다. 실행 파일은 CMake의 설치 RPATH 변환을 반영하여 비교했다. 경로 환경변수를 제거한 소비자가 stage의 0.38 라이브러리를 로딩하고 iillm이 LLM 런타임을 링크하지 않음을 확인했다. C++ 소비자는 0.38 헤더와 라이브러리로 함께 다시 빌드해야 한다.

종합 증거는 `build/memory-recall-verification.json`이다. 개별 결과는 `memory-recall-{release,sanitizer,package,linkage}.json`, `memory-recall-{source,installed}-native.json`, `memory-recall-{source,installed}-official-wire.json`과 해당 로그·CTest XML에 있다. 초기 실패와 수정 전 결과는 `.initial`, `.before-*`, `.stray-think-close`, `*-red*` 파일로 별도 보존한다.

검증 범위는 macOS arm64의 SDK와 별도 설치 소비자이다. 기본 홈 SDK는 0.36을 유지했으며 이번 단계에서 Society·Dreamscapes를 다시 패키징하지 않았다. Mac의 기존 앱 프로세스와 iPad의 앞선 설치 영수증이 현재 설치 상태와 일치함을 재확인했다. 기존 사용자 데몬 PID 14909를 유지했고 iPhone은 사용자 지시대로 제외했다. 전체 하네스는 31개 영역 중 23 partial·8 pending이며 세션 기록 검색, 자동 추출·dream 정리, 전문 에이전트 메모리, worktree·설정 계층·원격 동기화와 전체 앱·플랫폼 검증은 남아 있다.
