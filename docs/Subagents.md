...
`fork_context=true`이면 도구 묶음 시작 시점의 부모 스냅샷을 복사한다. 부모의 메시지·압축 경계·시스템 프롬프트를 유지하고, 미완료 부모 도구 호출에는 아직 결과를 모른다는 표시를 넣어 자식의 대화 형식을 완성한다. 부모 기록을 수정하거나 부모 도구를 재실행하지 않는다. 자식 요청에는 부모와 자식의 역할을 구분하는 지침을 덧붙인다. 0.47부터 문맥이 참조하는 artifact를 자식 소유로 복사하고 경로를 갱신한다. [SessionFork.md](SessionFork.md)를 따른다. 기본 분기는 같은 작업 디렉터리를 공유하므로 파일 변경도 공유한다.
